...
int main(int argc, char **argv) {
  ...
  Kokkos::initialize( argc, argv );
  {
    ...
    Kokkos::parallel_for("init_psi_p", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M_LEN,N_LEN}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
        psi(i,j) = a * std::sin((i + .5) * di) * std::sin((j + .5) * dj);
        p(i,j) = pcf * (std::cos(2. * (i) * di) + std::cos(2. * (j) * dj)) + 50000.;
    });
    
    // Initialize velocities
    Kokkos::parallel_for("init_u_v", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M,N}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
    ...
    Kokkos::parallel_for("init_old_arrays", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M_LEN,N_LEN}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
        uold(i,j) = u(i,j);
        vold(i,j) = v(i,j);
        pold(i,j) = p(i,j);
    });
    ...
    for (ncycle=1;ncycle<=ITMAX;++ncycle) {
      ...
      // Compute cu
      Kokkos::parallel_for("compute_cu", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({1,0}, {M_LEN,N}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
          cu(i,j) = 0.5 * (p(i,j) + p(i-1,j)) * u(i,j);
      });

      // Compute cv
      Kokkos::parallel_for("compute_cv", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,1}, {M,N_LEN}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
          cv(i,j) = 0.5 * (p(i,j) + p(i,j-1)) * v(i,j);
      });

      // Compute z
      Kokkos::parallel_for("compute_z", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({1,1}, {M_LEN,N_LEN}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
        z(i,j) = (fsdx * (v(i,j) - v(i-1,j)) - fsdy * (u(i,j) - u(i,j-1))) / (p(i-1,j-1) + p(i,j-1) + p(i,j) + p(i-1,j));
      });

      // Compute h
      Kokkos::parallel_for("compute_h", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M,N}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
      ...
      // Compute unew
      Kokkos::parallel_for("compute_unew", 
        Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({1,0}, {M_LEN,N}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
          unew(i,j) = uold(i,j) + tdts8 * (z(i,j+1) + z(i,j)) * (cv(i,j+1) + cv(i-1,j+1) + cv(i-1,j) + cv(i,j)) - tdtsdx * (h(i,j) - h(i-1,j));
      });

      // Compute vnew
      Kokkos::parallel_for("compute_vnew", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,1}, {M,N_LEN}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
          vnew(i,j) = vold(i,j) - tdts8 * (z(i+1,j) + z(i,j)) * (cu(i+1,j) + cu(i,j) + cu(i,j-1) + cu(i+1,j-1)) - tdtsdy * (h(i,j) - h(i,j-1));
      });

      // Compute pnew
      Kokkos::parallel_for("compute_pnew", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M,N}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
      ...
      if ( ncycle > 1 ) {
        Kokkos::parallel_for("time_smoothing_u", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M_LEN,N_LEN}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
            uold(i,j) = u(i,j) + alpha * (unew(i,j) - 2. * u(i,j) + uold(i,j));
        });

        Kokkos::parallel_for("time_smoothing_v", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M_LEN,N_LEN}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
            vold(i,j) = v(i,j) + alpha * (vnew(i,j) - 2. * v(i,j) + vold(i,j));
        });

        Kokkos::parallel_for("time_smoothing_p", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M_LEN,N_LEN}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
            pold(i,j) = p(i,j) + alpha * (pnew(i,j) - 2. * p(i,j) + pold(i,j));
        });
      }
      else {
        tdt = tdt + tdt;
        Kokkos::parallel_for("first_cycle_copy", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M_LEN,N_LEN}, {tilex, tiley}), KOKKOS_LAMBDA(const int i, const int j) {
          uold(i,j) = u(i,j);
          vold(i,j) = v(i,j);
          pold(i,j) = p(i,j);
        });
      ...
      if constexpr (!std::is_same_v<MemSpace, Kokkos::HostSpace>) {
        Kokkos::fence();
        Kokkos::parallel_for("swap_device_u_unew_views", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M_LEN,N_LEN}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
            auto temp = u(i,j);
            u(i,j) = unew(i,j);
            unew(i,j) = temp;
        });
        Kokkos::parallel_for("swap_device_v_vnew_views", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M_LEN,N_LEN}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
            auto temp = v(i,j);
            v(i,j) = vnew(i,j);
            vnew(i,j) = temp;
        });
        Kokkos::parallel_for("swap_device_p_pnew_views", Kokkos::MDRangePolicy<ExecSpace, Kokkos::Rank<2>>({0,0}, {M_LEN,N_LEN}, {tilex,tiley}), KOKKOS_LAMBDA(const int i, const int j) {
            auto temp = p(i,j);
            p(i,j) = pnew(i,j);
            pnew(i,j) = temp;
        });
