...
int main(int argc, char **argv) {
  ...
  Kokkos::initialize( argc, argv );
  {
    ...
    for (ncycle=1;ncycle<=ITMAX;++ncycle) {
      ...
      Kokkos::parallel_for("compute_cu_cv_z_h", teamPolicy(M, Kokkos::AUTO), KOKKOS_LAMBDA(const memberType& team) {
          const int i = team.league_rank();
          Kokkos::parallel_for(Kokkos::TeamThreadRange(team, N), [&](const int j) {
            double p_ij = p(i,j);
      ...
      Kokkos::parallel_for("compute_unew_vnew_pnew", teamPolicy(M, Kokkos::AUTO), KOKKOS_LAMBDA(const memberType& team) {
          const int i = team.league_rank();
          Kokkos::parallel_for(Kokkos::TeamThreadRange(team, N), [&](const int j) {
      ...
      if ( ncycle > 1 ) {
        Kokkos::parallel_for("time_smoothing_uvp", teamPolicy(M_LEN, Kokkos::AUTO), KOKKOS_LAMBDA(const memberType& team) {
          const int i = team.league_rank();
          Kokkos::parallel_for(Kokkos::TeamThreadRange(team, N_LEN), [&](const int j) {
            double u_ij = u(i,j);
            double v_ij = v(i,j);
