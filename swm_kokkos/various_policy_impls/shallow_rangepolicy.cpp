...
int main(int argc, char **argv) {
  ...
  Kokkos::initialize( argc, argv );
  {
    ...
    for (ncycle=1;ncycle<=ITMAX;++ncycle) {
      ...
      Kokkos::parallel_for("compute_cu_cv_z_h", Kokkos::RangePolicy<ExecSpace>(0, M), KOKKOS_LAMBDA(const int i) {
          for (int j = 0; j < N; ++j) {
            double p_ij = p(i,j);
      ...
      Kokkos::parallel_for("compute_unew_vnew_pnew", Kokkos::RangePolicy<ExecSpace>(0, M), KOKKOS_LAMBDA(const int i) {
          for (int j = 0; j < N; ++j) {
      ...
      if ( ncycle > 1 ) {
        Kokkos::parallel_for("time_smoothing_uvp", Kokkos::RangePolicy<ExecSpace>(0,M_LEN), KOKKOS_LAMBDA(const int i) {
