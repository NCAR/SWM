...
extern void write_uvp(double *u, double *v, double *p, int tM, int tN, int step, const char *suffix);
extern void write_cucvzh(double *cu, double *cv, double *z, double* h, int tM, int tN, int step, const char *suffix);
...
int main(int argc, char **argv) {
  ...
  for (ncycle=1;ncycle<=ITMAX;ncycle++) {
    if (VAL_OUT && ncycle <=4 ) write_uvp(u, v, p, M_LEN, N_LEN, ncycle-1,"init");
    ...
    if(VAL_OUT && ncycle <= 2) write_cucvzh(cu, cv, z, h, M_LEN, N_LEN, ncycle-1, "t100");
    ...
    if(VAL_OUT && ncycle <= 2) write_uvp(unew, vnew, pnew, M_LEN, N_LEN, ncycle-1, "t200");
  ...
  if(VAL_OUT){
    write_uvp(unew, vnew, pnew, M_LEN, N_LEN, ITMAX,"final");
...
void write_uvp(double *u, double *v, double *p, int tM, int tN, int step, const char *suffix) {
  write_to_file(u, tM, tN, (std::string("u.step") + std::to_string(step) + "." + suffix + ".bin").c_str());
  write_to_file(v, tM, tN, (std::string("v.step") + std::to_string(step) + "." + suffix + ".bin").c_str());
  write_to_file(p, tM, tN, (std::string("p.step") + std::to_string(step) + "." + suffix + ".bin").c_str());
}

void write_cucvzh(double *cu, double *cv, double *z, double* h, int tM, int tN, int step, const char *suffix) {
  write_to_file(cu, tM, tN, (std::string("cu.step") + std::to_string(step) + "." + suffix + ".bin").c_str());
  write_to_file(cv, tM, tN, (std::string("cv.step") + std::to_string(step) + "." + suffix + ".bin").c_str());
  write_to_file(z, tM, tN, (std::string("z.step") + std::to_string(step) + "." + suffix + ".bin").c_str());
  write_to_file(h, tM, tN, (std::string("h.step") + std::to_string(step) + "." + suffix + ".bin").c_str());
}
