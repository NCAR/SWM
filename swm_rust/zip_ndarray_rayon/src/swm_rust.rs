...
fn main() {
    ...
    let mut u = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut v = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut p = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut unew = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut vnew = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut pnew = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut uold = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut vold = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut pold = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut cu = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut cv = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut z = Array2::<f64>::zeros((M_LEN,N_LEN));
    let mut h = Array2::<f64>::zeros((M_LEN,N_LEN));

    // intermediate arrays using with Zip
    let mut term1 = Array2::<f64>::zeros((M,N));
    let mut term2 = Array2::<f64>::zeros((M,N));
    ...
    for ncycle in 1..=ITMAX {
        ...
        update_intermed_vars(&u, &v, &p, fsdx, fsdy, &mut cu, &mut cv, &mut z, &mut h, &mut term1, &mut term2);
        ...
        time_update_new_vars(&uold, &vold, &pold, &cu, &cv, &z, &h, tdts8, tdtsdx, tdtsdy, &mut unew, &mut vnew, &mut pnew, &mut term1, &mut term2);
    ...
    if SUCCINCT {
        println!("Version: zip ndarray rayon");
