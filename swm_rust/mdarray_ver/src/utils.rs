...
pub fn apply_uv_bcs(u: &mut DArray<f64, 2>, v: &mut DArray<f64, 2>) {
    for j in 0..N {
        u[[0,j]] = u[[M,j]];
        v[[M,j+1]] = v[[0,j+1]];
    }
...
pub fn update_intermed_vars(u: &DArray<f64, 2>, v: &DArray<f64, 2>, p: &DArray<f64, 2>, fsdx: f64, fsdy: f64, cu: &mut DArray<f64, 2>, cv: &mut DArray<f64, 2>, z: &mut DArray<f64, 2>, h: &mut DArray<f64, 2>) {
    for i in 0..M {
        for j in 0..N {
...
pub fn apply_intermed_bcs(cu: &mut DArray<f64, 2>, cv: &mut DArray<f64, 2>, z: &mut DArray<f64, 2>, h: &mut DArray<f64, 2>) {
    for j in 0..N {
        cu[[0,j]] = cu[[M,j]];
        cv[[M,j+1]] = cv[[0,j+1]];
        z[[0,j+1]] = z[[M,j+1]];
...
pub fn time_update_new_vars(uold: &DArray<f64, 2>, vold: &DArray<f64, 2>, pold: &DArray<f64, 2>, cu: &DArray<f64, 2>, cv: &DArray<f64, 2>, z: &DArray<f64, 2>, h: &DArray<f64, 2>, tdts8: f64, tdtsdx: f64, tdtsdy: f64, unew: &mut DArray<f64, 2>, vnew: &mut DArray<f64, 2>, pnew: &mut DArray<f64, 2>) {
    for i in 0..M {
        for j in 0..N {
            // let idx00 = ij_to_idx(i,j);
            // let idx01 = ij_to_idx(i,j+1);
...
pub fn apply_uvp_bcs(u: &mut DArray<f64, 2>, v: &mut DArray<f64, 2>, p: &mut DArray<f64, 2>) {
    for j in 0..N {
        u[[0,j]] = u[[M,j]];
        v[[M,j+1]] = v[[0,j+1]];
        p[[M,j]] = p[[0,j]];
...
pub fn smooth_update_old_vars(u: &DArray<f64, 2>, v: &DArray<f64, 2>, p: &DArray<f64, 2>, unew: &DArray<f64, 2>, vnew: &DArray<f64, 2>, pnew: &DArray<f64, 2>, uold: &mut DArray<f64, 2>, vold: &mut DArray<f64, 2>, pold: &mut DArray<f64, 2>, alpha: f64) {
    for i in 0..M_LEN {
        for j in 0..N_LEN {
...
pub fn print_data_to_file(pathname: &str, data: &DArray<f64, 2>) {
