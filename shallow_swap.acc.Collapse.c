...
#pragma acc parallel loop collapse(2) independent present(p[:SIZE]) deviceptr(psi)//private(a,di,dj,pcf)
  for (i=0;i<M_LEN;i++) {
    for (j=0;j<N_LEN;j++) {
      int idx = (i*N_LEN) + j;
...
#pragma acc parallel loop collapse(2) independent present(u[:SIZE],v[:SIZE]) deviceptr(psi)
  for (i=0;i<M;i++) {
    for (j=0;j<N;j++) {
...
#pragma acc parallel loop collapse(2) async(2) independent present(u[:SIZE],v[:SIZE],p[:SIZE]) deviceptr(uold,vold,pold) 
  for (i=0;i<M_LEN;i++) {
    for (j=0;j<N_LEN;j++) {
      int idx = i*N_LEN+j;
      uold[idx] = u[idx];
...
//#pragma acc exit data copyout(psi,p,a,pcf,di,dj,dx,dy,u,v,uold,pold,vold)
  ...
  for (ncycle=1;ncycle<=ITMAX;ncycle++) {
    ...
    c1 = wtime();  
#pragma acc parallel loop tile(64,64) independent present(p[:SIZE],u[:SIZE],v[:SIZE]) deviceptr(cu,cv,z,h)
    for (i=0;i<M;i++) {
      for (j=0;j<N;j++) {
        int idx00 = (i*N_LEN) + j;
        int idx01 = (i*N_LEN) + j+1;
...
#pragma acc parallel loop tile(64,64) independent present(unew[:SIZE],vnew[:SIZE],pnew[:SIZE]) deviceptr(cu,cv,z,h,uold,vold,pold)
    for (i=0;i<M;i++) {
      for (j=0;j<N;j++) {
        int idx00 = (i*N_LEN) + j;
        int idx01 = (i*N_LEN) + j+1;
