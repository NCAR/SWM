...
int main(int argc, char* argv[]) {
    ...
    for (int i=0; i<DOMAIN_SIZE; i++) dp[i]=p[tlmid][i]-50000.;
    ...
    for (int i=0; i<DOMAIN_SIZE; i++) dp[i]=p[tlnew][i]-50000.;

    char endfile[128] = "swm_end.";
    strcat(endfile,tail);
    outerr = output_csv_var(endfile, m, n, dp);

    if (outerr == 0) std::cout << "end file output complete" << std::endl;
